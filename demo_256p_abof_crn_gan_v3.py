...
checkpoint_name = "result_256p_crn_gan_v3"
...
if is_training:
    ...
    for epoch in range(1,61):
        ...
        for i in np.random.permutation(training_count):
            ...
            _,G_current,l0,GAN_ld,GAN_l,fake=sess.run([G_opt,G_loss,p0,GAN_loss_D,GAN_loss,fake_full_image],feed_dict={label:np.concatenate((label_images[i],np.expand_dims(1-np.sum(label_images[i],axis=3),axis=3)),axis=3),input_image:images[i],ref_image:fimage,y_true:const_tl,gt_label:tlabel,lr: 1e-4,weight: 2.0})#may try lr:min(1e-6*np.power(1.1,epoch-1),1e-4 if epoch>100 else 1e-3) in case lr:1e-4 is not good
