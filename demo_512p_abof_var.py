...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    p5=compute_error(vgg_real['conv5_2'],vgg_fake['conv5_2'],tf.image.resize_area(label,(sp//16,sp//16)))*10/1.5#weights lambda are collected at 100th epoch
...
checkpoint_name = "result_512p_abof_var"
...
for i in range(testing_count):
    ...
    upper=np.concatenate((output[0,:,:,:],output[1,:,:,:],output[2,:,:,:]),axis=1)
    bottom=np.concatenate((output[3,:,:,:],output[4,:,:,:],output[5,:,:,:]),axis=1)
    scipy.misc.toimage(np.concatenate((upper,bottom),axis=0),cmin=0,cmax=255).save(os.path.join(checkpoint_name, "final/%s_output.jpg" % file_name))
