...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    real_full_image = real_image + avg_image - 128
    fake_full_image = generator + avg_image - 128
    ...
    p0d=compute_error(vgg_real_diff['input'],vgg_fake_diff['input'],label)
    p1d=compute_error(vgg_real_diff['conv1_2'],vgg_fake_diff['conv1_2'],label)/1.6
    p2d=compute_error(vgg_real_diff['conv2_2'],vgg_fake_diff['conv2_2'],tf.image.resize_area(label,(sp//2,sp//2)))/2.3
    p3d=compute_error(vgg_real_diff['conv3_2'],vgg_fake_diff['conv3_2'],tf.image.resize_area(label,(sp//4,sp//4)))/1.8
    p4d=compute_error(vgg_real_diff['conv4_2'],vgg_fake_diff['conv4_2'],tf.image.resize_area(label,(sp//8,sp//8)))/2.8
    p5d=compute_error(vgg_real_diff['conv5_2'],vgg_fake_diff['conv5_2'],tf.image.resize_area(label,(sp//16,sp//16)))*10/0.8#weights lambda are collected at 100th epoch
    G_loss=p0+p1+p2+p3+p4+p5+p0d+p1d+p2d+p3d+p4d+p5d
...
checkpoint_name = "result_256p_abof_diffex_fullimg"
...
dir_label = "data/abof256x256/label/"
#dir_image = "data/abof256x256/image/"
dir_diff_image = "data/abof256x256/diff/"
dir_self_image = "data/abof256x256/self/"
file_list = []
...
if is_training:
    ...
    diff_images=[None]*training_count
    ...
    for epoch in range(1,201):
        ...
        for i in np.random.permutation(training_count):
            ...
            st=time.time()
            cnt+=1
            if diff_images[i] is None:
                label_images[i]=helper.get_index_semantic_map(os.path.join(dir_label, file_name), n_classes)#training label
                self_images[i]=np.expand_dims(np.float32(scipy.misc.imread(os.path.join(dir_self_image, file_name))),axis=0)#training average image
                diff_images[i]=np.expand_dims(np.float32(scipy.misc.imread(os.path.join(dir_diff_image, file_name))),axis=0)#training image
            _,G_current,l0,l1,l2,l3,l4,l5=sess.run([G_opt,G_loss,p0,p1,p2,p3,p4,p5],feed_dict={label:np.concatenate((label_images[i],np.expand_dims(1-np.sum(label_images[i],axis=3),axis=3)),axis=3),real_image:diff_images[i],avg_image:self_images[i],lr: 1e-4})#may try lr:min(1e-6*np.power(1.1,epoch-1),1e-4 if epoch>100 else 1e-3) in case lr:1e-4 is not good
            g_loss[i]=G_current
        ...
        for i in range(testing_count):
            ...
            scipy.misc.toimage(output[0,:,:,:],cmin=0,cmax=255).save(os.path.join(checkpoint_name, "%04d/%s_diff_output.png"%(epoch,file_name)))
            scipy.misc.toimage(full_image,cmin=0,cmax=255).save(os.path.join(checkpoint_name, "%04d/%s_output.jpg"%(epoch,file_name)))
...
for i in range(testing_count):
    ...
    output=np.minimum(np.maximum(output, 0.0), 255.0)
    full_image=np.minimum(np.maximum(full_image,0.0),255.0)
    scipy.misc.toimage(output[0,:,:,:],cmin=0,cmax=255).save(os.path.join(checkpoint_name, "final/%s_diff_output.png"%(file_name)))
    scipy.misc.toimage(full_image,cmin=0,cmax=255).save(os.path.join(checkpoint_name, "final/%s_output.jpg"%(file_name)))
