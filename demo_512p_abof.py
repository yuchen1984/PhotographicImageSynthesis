...
if is_training:
    ...
    for epoch in range(1,21):
        if os.path.isdir("result_512p_abof/%04d"%epoch):
            continue
        cnt=0
        for i in np.random.permutation(training_count):
            ...
            if input_images[i] is None:
                label_images[i]=helper.get_index_semantic_map(os.path.join(dir_label, ind), n_classes)#training label
                input_images[i]=np.expand_dims(np.float32(scipy.misc.imread(os.path.join(dir_image, ind.replace('.png','.jpg')))),axis=0)#training image
            _,G_current,l0,l1,l2,l3,l4,l5=sess.run([G_opt,G_loss,p0,p1,p2,p3,p4,p5],feed_dict={label:np.concatenate((label_images[i],np.expand_dims(1-np.sum(label_images[i],axis=3),axis=3)),axis=3),real_image:input_images[i],lr:1e-4})
            g_loss[i]=G_current
        ...
        os.makedirs("result_512p_abof/%04d"%epoch)
        target=open("result_512p_abof/%04d/score.txt"%epoch,'w')
        target.write("%f"%np.mean(g_loss[np.where(g_loss)]))
        target.close()
        saver.save(sess,"result_512p_abof/model.ckpt")
        if epoch%20==0:
            saver.save(sess,"result_512p_abof/%04d/model.ckpt"%epoch)
        for i in range(testing_count):
            ind = file_list[training_count + i]
            if not os.path.isfile(os.path.join(dir_label, ind)):#test label
                continue
            semantic=helper.get_index_semantic_map(os.path.join(dir_label, ind), n_classes)#test label
            ...
            scipy.misc.toimage(output[0,:,:,:],cmin=0,cmax=255).save("result_512p_abof/%04d/%s_output.jpg"%(epoch,ind))

if not os.path.isdir("result_512p_abof/final"):
    os.makedirs("result_512p_abof/final")
...
for i in range(testing_count):
    ...
    output=sess.run(generator,feed_dict={label:np.concatenate((semantic,np.expand_dims(1-np.sum(semantic,axis=3),axis=3)),axis=3)})
    output=np.minimum(np.maximum(output,0.0),255.0)
    scipy.misc.toimage(output[0,:,:,:],cmin=0,cmax=255).save("result_512p_abof/final/%s_output.jpg"%ind)
