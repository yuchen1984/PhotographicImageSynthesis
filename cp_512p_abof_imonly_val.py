...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    fake_full_image = tf.clip_by_value(fake_full_image, tf.cast(0.0, dtype=tf.float32), tf.cast(255.0, dtype=tf.float32))
    ...
    p5=compute_error(vgg_real['conv5_2'],vgg_fake['conv5_2'])*10/1.5#weights lambda are collected at 100th epoch
...
pretrained_checkpoint_name = "result_256p_cp_imonly"
checkpoint_name = "result_512p_cp_imonly"
...
if is_training:
    ...
    for epoch in range(1,41):
        ...
        for i in np.random.permutation(training_count):
            ...
            _,G_current,l0,l1,l2,l3,l4,l5=sess.run([G_opt,G_loss,p0,p1,p2,p3,p4,p5],feed_dict={real_image:image,avg_image:self_image,lr: 1e-4})#may try lr:min(1e-6*np.power(1.1,epoch-1),1e-4 if epoch>100 else 1e-3) in case lr:1e-4 is not good
            g_loss[i]=G_current
        ...
        for i in range(testing_count):
            ...
            v_loss,output=sess.run([G_loss,generator],feed_dict={real_image:val_image,avg_image:test_self_image})
            val_loss[i]=v_loss
