...
optimizer_config = "./momentum_ft.json"
...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    ref_image=tf.placeholder(tf.float32,[None,sp,sp,3])
    y_true=tf.placeholder(tf.float32,[None,2])
    gt_label=tf.placeholder(tf.float32,[None,2])
    weight=tf.placeholder(tf.float32)

    generator=recursive_generator(label,input_image,sp)
    
    fake_full_image = generator + input_image - 128
    ...
    disc_output_G = create_dnn(network_type='googlenet', input_datas=[normalized_fake_full_image], is_training=True, task_configs=tasks,
                             weight_sharing = 'all', data_scope = "recolour_disc", optimizer_config=optimizer_config)
    ...
    pred = tf.clip_by_value(disc_output_G, tf.cast(_EPSILON, dtype=_FLOATX), tf.cast(1.-_EPSILON, dtype=_FLOATX))
    GAN_loss = categorical_crossentropy(y_true, pred) * weight
    
    normalized_ref_image = (ref_image / 255.0 - 0.723029862732) / 0.269584051053
    disc_output_D = create_dnn(network_type='googlenet', input_datas=[normalized_ref_image], is_training=True, task_configs=tasks,
                             weight_sharing = 'all', data_scope = "recolour_disc", optimizer_config=optimizer_config, force_reuse=True)
    pred_D = tf.clip_by_value(disc_output_D, tf.cast(_EPSILON, dtype=_FLOATX), tf.cast(1.-_EPSILON, dtype=_FLOATX))
    GAN_loss_D = categorical_crossentropy(gt_label, pred_D) * weight


    #GAN_loss = tf.log(pred[1]) * loss_weight
    #print get_incoming_shape(pred), get_incoming_shape(GAN_loss), get_incoming_shape(p5)
    #G_loss=p0+p4+p5+GAN_loss+GAN_loss_D
    G_loss=p0+GAN_loss+GAN_loss_D
...
pretrained_checkpoint_name = "result_256p_crn_gan_v3"
checkpoint_name = "result_512p_crn_gan_v3"
...
if ckpt:
  ...
  if ckpt_prev:
    saver=tf.train.Saver(var_list=[var for var in tf.trainable_variables() if not var.name.startswith('g_512') and not var.name.startswith('recolour_disc')])
    print('loaded '+ckpt_prev.model_checkpoint_path)
    saver.restore(sess,ckpt_prev.model_checkpoint_path)
#else:
  disc_checkpoint_name = "disc_models_512"
...
dir_in_l = 'abof-site-images-graysegmentation-resize512/'
dir_in_im = 'abof-site-images-resize512/'
dir_in_rc = 'abof-site-images-randrecolour-resize512/'
dir_out = 'abof-recolour-disc-x4-512/'
...
if is_training:
    const_tl=np.zeros((1,2),dtype=np.float32)
    const_tl[0,1] = 1.0
    ...
    for epoch in range(1,61):
        ...
        for i in np.random.permutation(training_count):
            ...
            tlabel[0,0] = 1.0 - float(training_tlabels[i])
            tlabel[0,1] = float(training_tlabels[i])
            st=time.time()
            cnt+=1
            label_image=helper.get_index_semantic_map(label_file_name, n_classes)#training label
            image=np.expand_dims(np.float32(scipy.misc.imread(file_name)),axis=0)#training image
            if fimage is None:
                  fimage = image
                #print fimage.shape

            _,G_current,l0,GAN_ld,GAN_l,fake=sess.run([G_opt,G_loss,p0,GAN_loss_D,GAN_loss,fake_full_image],feed_dict={label:np.concatenate((label_image,np.expand_dims(1-np.sum(label_image,axis=3),axis=3)),axis=3),input_image:image,ref_image:fimage,y_true:const_tl,gt_label:tlabel,lr: 1e-4,weight: 2.0})#may try lr:min(1e-6*np.power(1.1,epoch-1),1e-4 if epoch>100 else 1e-3) in case lr:1e-4 is not good
            
            fimage = image if training_tlabels[i] == 1 else fake
            fimage=np.minimum(np.maximum(fimage,0.0),255.0)
            g_loss[i]=G_current
            print("%d %d %.2f %.2f %.2f %.2f %.2f"%(epoch,cnt,np.mean(g_loss[np.where(g_loss)]),np.mean(l0),np.mean(GAN_l),np.mean(GAN_ld),time.time()-st))
        ...
        for i in range(testing_count):
            ...
            scipy.misc.toimage(output[0,:,:,:],cmin=0,cmax=255).save(os.path.join(checkpoint_name, "%04d/diff_%s"%(epoch,file_name_base)))
...
for i in range(testing_count):
    ...
    output=np.minimum(np.maximum(output, 0.0), 255.0)
    full_image=np.minimum(np.maximum(full_image,0.0),255.0)
    scipy.misc.toimage(output[0,:,:,:],cmin=0,cmax=255).save(os.path.join(checkpoint_name, "final/diff_%s"%(file_name_base)))
