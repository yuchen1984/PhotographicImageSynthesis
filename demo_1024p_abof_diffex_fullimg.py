...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    p1d=compute_error(vgg_real_diff['conv1_2'],vgg_fake_diff['conv1_2'],label)/2.6
    p2d=compute_error(vgg_real_diff['conv2_2'],vgg_fake_diff['conv2_2'],tf.image.resize_area(label,(sp//2,sp//2)))/4.8
    p3d=compute_error(vgg_real_diff['conv3_2'],vgg_fake_diff['conv3_2'],tf.image.resize_area(label,(sp//4,sp//4)))/3.7
    p4d=compute_error(vgg_real_diff['conv4_2'],vgg_fake_diff['conv4_2'],tf.image.resize_area(label,(sp//8,sp//8)))/5.6
...
pretrained_checkpoint_name = "result_512p_abof_diffex_fullimg"
checkpoint_name = "result_1024p_abof_diffex_fullimg"
...
#dir_diff_image = "data/abof1024x1024/image/"
dir_diff_image = "data/abof1024x1024/diff/"
dir_self_image = "data/abof1024x1024/self/"
file_list = []
...
if is_training:
    ...
    for epoch in range(1,21):
        ...
        for i in np.random.permutation(training_count):
            ...
            diff_image=np.expand_dims(np.float32(scipy.misc.imread(os.path.join(dir_diff_image, file_name))),axis=0)#training image
        ...
        for i in range(testing_count):
            ...
            test_avg_image=np.expand_dims(np.float32(scipy.misc.imread(os.path.join(dir_self_image, file_name))),axis=0)#test average image
