# ***************
# deeplab_lfov_upsample.py
# ***************
# Tflearn re-implementation of DeepLab-LargeFOV model, based on the original TensorFlow
# implemenation in https://github.com/DrSleep/tensorflow-deeplab-lfov. This version is modified
# to include upsampling layers for the label prediction.
# (Author:  Yu Chen).
# ***************
...
def create_network(input_data,
  ...
  network = conv_2d(network, 1024, 3, activation='relu', name='fc6', dilation=12, restore=(retrain_final_layers < 5))
  network = dropout(network, keep_prob, name='dropout1')
  if remove_final_layers < 4:
    network = conv_2d(network, 1024, 1, activation='relu', name='fc7', restore=(retrain_final_layers < 4))
    network = dropout(network, keep_prob, name='dropout2')

  if remove_final_layers < 3:
    network = upsample_2d(network, 2, name='upsample1')
    network = conv_2d(network, 256, 3, activation='relu', name='conv8_1', restore=(retrain_final_layers < 3))
    network = conv_2d(network, 256, 1, activation='relu', name='conv8_2', restore=(retrain_final_layers < 3))
    network = dropout(network, keep_prob, name='dropout3')
  
  if remove_final_layers < 2:
    network = upsample_2d(network, 2, name='upsample2')
    network = conv_2d(network, 128, 3, activation='relu', name='conv9_1', restore=(retrain_final_layers < 2))
    network = conv_2d(network, 64, 1, activation='relu', name='conv9_2', restore=(retrain_final_layers < 2))
    network = dropout(network, keep_prob, name='dropout4')

  if remove_final_layers < 1:
    network = upsample_2d(network, 2, name='upsample3')
    network = conv_2d(network, num_classes, 3, activation='linear', name='conv10', restore=(retrain_final_layers < 1))
