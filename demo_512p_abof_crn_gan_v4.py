...
sp=512#spatial resolution: 256x256
...
pretrained_checkpoint_name = "result_256p_crn_gan_v4"
checkpoint_name = "result_512p_crn_gan_v4"
...
if ckpt:
  ...
  if ckpt_prev:
    saver=tf.train.Saver(var_list=[var for var in tf.trainable_variables() if not var.name.startswith('g_512') and not var.name.startswith('recolour_disc/FullyConnected')])
...
if is_training:
    ...
    for epoch in range(1,61):
        ...
        for i in np.random.permutation(training_count):
            ...
            _,G_current,l0,l4,l5,GAN_ld,GAN_l,fake=sess.run([G_opt,G_loss,p0,p4,p5,GAN_loss_D,GAN_loss,fake_full_image],feed_dict={label:np.concatenate((label_images[i],np.expand_dims(1-np.sum(label_images[i],axis=3),axis=3)),axis=3),input_image:images[i],ref_image:fimage,y_true:const_tl,gt_label:tlabel,lr: 1e-4,weight: 2.0})#may try lr:min(1e-6*np.power(1.1,epoch-1),1e-4 if epoch>100 else 1e-3) in case lr:1e-4 is not good
            
            fimage = images[i] if training_tlabels[i] == 1 else fake
            fimage=np.minimum(np.maximum(fimage,0.0),255.0)
            g_loss[i]=G_current
            print("%d %d %.2f %.2f %.2f %.2f %.2f %.2f %.2f"%(epoch,cnt,np.mean(g_loss[np.where(g_loss)]),np.mean(l0),np.mean(l4),np.mean(l5),np.mean(GAN_l),np.mean(GAN_ld),time.time()-st))
