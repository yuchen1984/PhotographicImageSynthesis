...
pretrained_checkpoint_name = "result_1024p_abof_avg_single"
checkpoint_name = "result_1024p_abof_avg_single_ft"
...
if is_training:
    ...
    for epoch in range(35,61):
        ...
        for i in np.random.permutation(training_count):
            ...
            _,G_current,l0,l1,l2,l3,l4,l5=sess.run([G_opt,G_loss,p0,p1,p2,p3,p4,p5],feed_dict={label:np.concatenate((label_image,np.expand_dims(1-np.sum(label_image,axis=3),axis=3)),axis=3),real_image:input_image,avg_image:average_image,lr:0.6e-5})
            g_loss[i]=G_current
