...
checkpoint_name = "result_256p_cp_diff"
...
dir_label = "/data/metail_garments/cp-mask-resize256/"
dir_image = "/data/metail_garments/cp-diff-resize256/"
...
if is_training:
    ...
    for epoch in range(1,201):
        ...
        for i in np.random.permutation(training_count):
            ...
            cnt+=1
            if images[i] is None:
                label_images[i]=helper.get_index_semantic_map(os.path.join(dir_label, file_name.replace('_ns','_mask')), n_classes)#training label
                ...
                images[i]=np.expand_dims(np.float32(scipy.misc.imread(os.path.join(dir_image, file_name.replace('_ns','_diff')))),axis=0)#training image
            _,G_current,l0,l1,l2,l3,l4,l5=sess.run([G_opt,G_loss,p0,p1,p2,p3,p4,p5],feed_dict={label:np.concatenate((label_images[i],np.expand_dims(1-np.sum(label_images[i],axis=3),axis=3)),axis=3),real_image:images[i],avg_image:self_images[i],lr: 1e-4})#may try lr:min(1e-6*np.power(1.1,epoch-1),1e-4 if epoch>100 else 1e-3) in case lr:1e-4 is not good
            g_loss[i]=G_current
